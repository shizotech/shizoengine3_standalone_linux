...
vec4 vibrance(vec4 inCol, float vibrance) //r,g,b 0.0 to 1.0,  vibrance 1.0 no change, 0.0 image B&W.
{
    ...
    else // vibrance > 1.0
    {
        ...
        a = mix(a, a * (hue_a * 0.67 + 0.33), smoothstep(0.0, 1.0, hue_a));									
        a *= (vibrance - 1.0);
        ...
        a = float(bitwiseAnd(use , 1)); // i == 0;
        use >>= 1;
        inCol.rgb += a * vec3(v, p3, p1);
 
        a = float(bitwiseAnd(use , 1)); // i == 1;
        use >>= 1;
        inCol.rgb += a * vec3(p2, v, p1); 

        a = float( bitwiseAnd(use,1)); // i == 2;
        use >>= 1;
        inCol.rgb += a * vec3(p1, v, p3);

        a = float(bitwiseAnd(use, 1)); // i == 3;
        use >>= 1;
        inCol.rgb += a * vec3(p1, p2, v);

        a = float(bitwiseAnd(use, 1)); // i == 4;
        use >>= 1;
        inCol.rgb += a * vec3(p3, p1, v);

        a = float(bitwiseAnd(use, 1)); // i == 5;
        use >>= 1;
...
{
    // uniforms
    float _vibrance = vibrance_color;
    float _hue = hue_rotate; 
    
    vec4 color = texture( iChannel0, fragCoord/iResolution.xy );
    
    fragColor = saturationMatrix(saturation) * color; 
    brightnessAdjust(fragColor, brightness); 
    contrastAdjust(fragColor, contrast); 
