...
//@slider min=0.0 max=1.0 value=0.0
uniform float smoothness_in;   // Invertierter Regler: 0 = gerade glatte Kante, 1 = harte Kante (tangential, Rotationsrichtung)
//@slider min=0.0 max=1.0 value=0.0
uniform float smoothness_out;  // Invertierter Regler: 0 = gerade glatte Kante, 1 = harte Kante (hintere Hälfte)
//@slider min=0.0 max=0.5 value=0.0
...
float drawBar(vec2 p, int idx, float time) {
    ...
    // Dabei auch die Projektion merken, um die seitliche Seite (in Bewegungs-
    // richtung vs. hinten) bestimmen zu können.
    float best = 1e9;
    vec2 bestProj = p;
    const int SEGS = 12;
    vec2 prev = A;
    for (int s = 1; s <= SEGS; s++) {
        ...
        // Distanz + Projektion des nächsten Punktes auf das Segment
        vec2 ab = pt - prev;
        float tt = clamp(dot(p - prev, ab) / max(dot(ab, ab), 1e-6), 0.0, 1.0);
        vec2 proj = prev + tt * ab;
    ...
    // Bewegungsrichtung ist tangential (perp), da der Balken um den Ursprung
    // rotiert (CCW -> time steigt). Seite relativ zur Balkenachse:
    //   vor = +perp  -> Vorderseite (in Bewegungsrichtung, "in")
    //   zurück = -perp -> Rückseite ("out")
    float side = dot(p - bestProj, perp);

    // Breite des Balkens aus der bar_width-Einstellung
    float w = bar_width;

    // In-Hälfte: invertierter Regler -> smoothness_in=0 = weiche glatte Kante,
    // smoothness_in=1 = harte Kante (Wert wird invertiert verwendet).
    float inHard  = smoothstep(w, w * 0.4, best);
    float inSoft  = smoothstep(w * 0.5, w * 0.1, best);
    float inVal   = mix(inSoft, inHard, smoothness_in);

    // Out-Hälfte: Weichheit aus invertiertem smoothness_out, Falloff zusätzlich
    // um tail_length verlängert.
    float outHard = smoothstep(w + tail_length, (w + tail_length) * 0.4, best);
    float outSoft = smoothstep((w + tail_length) * 0.5, (w + tail_length) * 0.1, best);
    float outVal  = mix(outSoft, outHard, smoothness_out);

    return (side >= 0.0) ? inVal : outVal;
}
