...
//@slider min=0.0 max=5.0 value=2.133333
...
float dustLayer(vec2 p){
    ...
    return smoothstep(0.,1.,id)*particle((fract(p)-vec2(.5+.4*cos(id*animTime),.5+.4*sin(.8*id*animTime)))*rotM(id*360.)/vec2(cos(.5*id*animTime),1));
}
