...
float backstarsf(vec2 UV, float mult)
{
    ...
    vec2 hash = Hash22(id);
    vec2 circlepos = hash;
    float circle = Dist2(vec2(gv), circlepos);
    float circler = 0.05;
    float star;
    if (circle < circler)
    {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (m == 1.) // outside mandelbrot - leaves
    {
        ...
        float mixTime = 0.;
        vec3 amix = mix(a1, a2, mixTime);
        vec3 bmix = mix(b1, b2, mixTime);
        vec3 cmix = mix(c1, c2, mixTime);
        vec3 dmix = mix(d1, d2, mixTime);
    ...
    else // inside mandelbrot
    {
        ...
        if (circle < circler)
        {
            float dist2_ = Dist2(uv, vec2(0.));
            vec3 sun = vec3(0.5 + 0.5 * cos(time * 0.771 + dist2_ * 10.), 0.1, 0.5 + 0.5 * sin(time * 0.471 + dist2_ * 10.));
