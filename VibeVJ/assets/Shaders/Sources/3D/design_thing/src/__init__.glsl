...
float hash12(vec2 x) { return fract(sin(dot(x, vec2(43.5287, 41.12871))) * 523.582); }
vec2 hash21(float x) { return fract(sin(x * vec2(24.0181, 52.1984)) * 5081.4972); }
float hash11(float x) { return fract(sin(x * 42.146291) * 4215.4827); }
vec2 hash22(vec2 x) { return fract(sin(x * mat2x2(24.4372, 12.47864, 32.3874, 29.4873)) * 4762.832); }
mat2x2 rotationMatrix(in float angle) { return mat2x2(-cos(angle), sin(angle), -sin(angle), -cos(angle)); }
...
float dashedCircle(vec2 uv, in float radius, in float width, in float density) {
    ...
    angle /= TWO_PI; angle = fract(angle * density);
...
float writings(vec2 uv, in float linesCount, in float maxLen, in float height, in float hash) {
    float mul = smoothstep(0.0, 0.01, uv.x) * smoothstep(0.0, 0.01, uv.y) * smoothstep(height, height - 0.001, uv.y);
...
float writingsGrid(vec2 uv, in vec2 gridSpace, in float scale) {
    ...
    fractUV *= scale;
    return writings(fractUV, 10.0 * hash12(floorUV) + 5.0, 1.0, hash12(floorUV + 0.1) * 0.6, 1.0);
}
...
uniform int angle_mod;
//@slider min=0.0 max=1.0 value=0.0
uniform float cangle_x;
//@slider min=0.0 max=1.0 value=0.0
uniform float cangle_y;
//@slider min=0.0 max=1.0 value=0.0
uniform float cangle_z;
//@slider min=0.0 max=1.0 value=1.0
uniform float pos_mod;
//@button
uniform bool blur_enabled;
//@button
uniform bool smoothstep_variant;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(angle_mod == 0) {
        angleX = cangle_x * 2.0 * PI;
        angleY = cangle_y * 2.0 * PI;
        angleZ = cangle_z * 2.0 * PI;
    }
    ...
    vec3 cameraPosition = vec3(sin(time) * pos_mod, -3.1 - sin(angleTime) * 0.2 * pos_mod, cos(time) * pos_mod);
